...
def baseline(net, serv_matrix: dict, pro_serv_num: int, traffic_num: int, total_pro_bw: int, total_traffic_bw: int) ->dict:
    ...
    print('----->1:1 baseline algorithm is planning services...')
    serv_path = dict()
    ...
    for service in serv_matrix:
        ...
        if service['type'] == 'P':
            ...
            allo_flag, work_path, work_wave_id, backup_path, backup_wave_id, id = ksp_FF(id_set, serv_matrix, serv_path, service, net, k=3, flag=True)
            if id:
                id_set.append(id)
            ...
            if allo_flag == True:
                ...
                total_allo_pro_serv_bw = total_allo_pro_serv_bw + (service['bw'] * len(backup_path))
            ...
            allo_flag, traffic_path, traffic_wave_id, _, _ = ksp_FF(service, net, k=5, flag=False)
            if allo_flag ==True:
    ...
    for k in id_set:
        #print(k)
        #print(serv_matrix)
        temp_bw = serv_matrix[k]['bw']
        temp_bppath = serv_path[k]['backup_path'][0]
        total_allo_pro_serv_bw = total_allo_pro_serv_bw - temp_bw * (len(temp_bppath))
    ...
    print('Resource utilization rate of traffic:{:.2%}'.format(traffic_resource_utilization_rate), '\n')
    #print(net.network_status)

    return serv_path

def ksp_FF(id_set, serv_matrix, serv_path, single_serv, net, k, flag=True):
    """
    ...
    id = None
    m = None
    for s in serv_matrix[:single_serv['id']]:
        if (single_serv['src_dst'] == s['src_dst']) & (single_serv['bw'] == s['bw']):
            if serv_path[s['id']]:
                if s['id'] not in id_set :
                    work_path = serv_path[s['id']]['backup_path']
                    wp = work_path[0]
                    n = 0
                    for t in wp:
                        ...
                        if n == 0:
                            temp.append(int(t[:index1]))
                            temp.append(int(t[index2+1:]))
                        else:
                            temp.append(int(t[index2+1:]))
                        n = n+1
                    id = s['id']
    ...
    for w_k_l in work_ksp_list:
        ...
        if f == 0:
            ...
            work_path_avail_flag = True
            work_wave_avail_id = work_path[1]
            work_link_key_list = work_path[0]
                ...
                for b_k_l in backup_ksp_list:
                    ...
                    if backup_path_avail_flag == True:
                        ...
                        for key in backup_link_key_list:
                            net.network_status[key][backup_wave_avail_id] = net.network_status[key][backup_wave_avail_id] - single_serv['bw']
    ...
    return allo_flag, work_path, work_wave_avail_id, backup_path, backup_wave_avail_id, m
